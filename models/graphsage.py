...
class GCN(nn.Module):
	...
	def __init__(self, num_classes, enc):
		...
		self.enc = enc
		self.xent = nn.CrossEntropyLoss()
		self.weight = nn.Parameter(torch.FloatTensor(num_classes, enc.embed_dim)).cuda(1)
		init.xavier_uniform_(self.weight)
		self.pos_vector = None
		self.neg_vector = None
	...
	def forward(self, nodes, train_flag=True):
		if train_flag:
			embeds = self.enc(nodes, train_flag).cuda(1)
			scores = self.weight.mm(embeds.cuda(1))
		else:
...
class GCNEncoder(nn.Module):
	...
	def __init__(self, features, feature_dim,
		...
		self.aggregator.cuda = cuda
		self.weight = nn.Parameter(
			torch.FloatTensor(embed_dim, self.feat_dim )).cuda(1)
		init.xavier_uniform_(self.weight)
	...
	def forward(self, nodes, train_flag=True):
		...
		self_feats = self.features(index).cuda(1)

		combined = F.relu(self.weight.mm(neigh_feats.t().cuda(1)))
		if not train_flag:
	...
	def update_label_vector(self, x):
		...
		if isinstance(x, torch.Tensor):
			...
			x_pos = x(self.pos_index).cuda(1)
			x_neg = x(self.neg_index).cuda(1)
		if self.pos_vector is None:
			self.pos_vector = torch.mean(x_pos, dim=0, keepdim=True).detach()
			self.neg_vector = torch.mean(x_neg, dim=0, keepdim=True).detach()
		else:
